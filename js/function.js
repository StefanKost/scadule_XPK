function clickSubject(elem){
    ...
    var name_teacher = ''; var name_teacher2 = '';
    ...
    audience[0].value=''; audience2[0].value='';
    ...
    if (potok.length>0) potok.forEach(function (p){
        ...
        predmets.forEach(function(d){
            if((d['id']==prev_subject_id)&&(d['group_id']==p)) { d['count_first'] = d['count_first'] + 0.5; }
        });
    });


    teacher[0].innerHTML = name_teacher;
    audience[0].readOnly=false; audience2[0].readOnly=true;
    ...
    if (dual) addDualSubjects(id_subject, name_teacher, group, tr, tr2, elem);
        ...
        for(i=day.start; i<=day.end; i++){
                ...
                if (sel.options[sel.selectedIndex].value==elem.options[elem.selectedIndex].value) {povtor_teacher=true; break;}
        }
...
function getSubjects(elem){
    if ((elem.parentNode.id.replace(/[^0-9]/g,'')!=group)||(elem.id.replace(/[^-0-9]/g,'')!=para)) {
        ...
        if (elem.options[elem.selectedIndex].value==-1) {prev_subject_id=-1; prev_arr_index=''; prev_pract=false;}
        else {prev_subject_id=elem.options[elem.selectedIndex].value;
        ...
        if (elem.options.length==1) {
                    ...
                    for(i=day.start; i<=day.end; i++)
                        if(i!=row_id){
                            ...
                            if (elem.parentNode.parentNode.parentNode.parentNode.id=="numerator")
                                ...
                                sel = document.getElementsByClassName("rozklad")[1].getElementsByClassName("row:"+i)[0].getElementsByClassName("id_group:"+group)[0].getElementsByClassName("subjects")[0];
                            if (sel.options[sel.selectedIndex].value==dat['id']) {proverka=false; break;}
                        }
            ...
            if (dual) {
                ...
                for(var i=0; i<arr_subj.length; i++){
                    if((arr_subj[i].parentNode.className!="id_group:"+group)&&(arr_subj[i].options[arr_subj[i].selectedIndex].value==elem.options[elem.selectedIndex].value)) {dual=true; break;}
                }
            }

            var arr_teach; var arr_teach2;
            if(elem.parentNode.parentNode.parentNode.parentNode.id=="numerator") {
                ...
                for(i=day.start; i<=day.end; i++)
                    if(i!=row_id){
                        ...
                        if(elem.parentNode.parentNode.parentNode.parentNode.id=="numerator")
                            ...
                            sel = document.getElementsByClassName("rozklad")[1].getElementsByClassName("row:"+i)[0].getElementsByClassName("id_group:"+group)[0].getElementsByClassName("subjects")[0];
                        if (sel.options[sel.selectedIndex].value==dat['id']) {add=false; break;}
...
function rowDay(row_id){
    ...
    for (var i=0; i<19; i=i+4){
        if ((row_id>=i)&&(row_id<i+4)){ day_row_start=i; day_row_end=i+4-1; }
    }
    var day_row = {};
    ...
    function addDualSubjects(id, teacher_name, group, tr, tr2, elem){
        var index=0;
        for(var i=0; i<predmets.length; i++) if((predmets[i]['id']==id)&&(predmets[i]['group_id']==group)){ index=i; break; }
        if(predmets[index]['count_first']>0) {alert(predmets[index]['count_first']+' - '+predmets[index]['subject_title']);
        if (confirm("Якщо це лекційне занняття натисніть 'ОК', практичне - 'Скасувати'")) {
            var conf=true;
            for(i=0; i<predmets.length; i++)  if((predmets[i]['id']==id)&&(predmets[i]['group_id']!=group)) if(predmets[i]['count_first']!=predmets[index]['count_first']) {conf=false; break;}
                ...
                for(i=0; i<groups_ids.length; i++) {
                    for(var j=day.start; j<=day.end; j++){
                        ...
                        if (sel.options[sel.selectedIndex].value==elem.options[elem.selectedIndex].value) {perevirka=false; break;}
                    }
                }
                ...
                if (perevirka) {
                    ...
                    if((predmets[index]['count_first']<1)||(perevirka_teacher==false)){
                        ...
                        for (i=0; i<arr_subject.length; i++) {
                            ...
                            var ind;
                            for(var g=0; g<predmets.length; g++) if((predmets[g]['id']==id)&&(predmets[g]['group_id']==gr)){ ind=g; break; }
                ...
                if ((predmets[index]['count_first']>=1)&&(tr2[1].getElementsByClassName("teacher:"+group)[0].innerHTML=='')){
                    ...
                    for(i=day.start; i<=day.end; i++){
                        ...
                        if(elem.parentNode.parentNode.parentNode.parentNode.id=="numerator")
                            ...
                            sel = document.getElementsByClassName("rozklad")[0].getElementsByClassName("row:"+i)[0].getElementsByClassName("id_group:"+group)[0].getElementsByClassName("subjects")[0];
                        if (sel.options[sel.selectedIndex].value==elem.options[elem.selectedIndex].value) {perevirka_teacher=false; break;}
                    }
            ...
            if(teacher2_perevirka) {
                ...
                for(i=day.start; i<=day.end; i++){
                    ...
                    if(elem.parentNode.parentNode.parentNode.parentNode.id=="numerator")
                        ...
                        sel = document.getElementsByClassName("rozklad")[0].getElementsByClassName("row:"+i)[0].getElementsByClassName("id_group:"+group)[0].getElementsByClassName("subjects")[0];
                    if (sel.options[sel.selectedIndex].value==elem.options[elem.selectedIndex].value) {teachers_perevirka=false; break; }
                }
            ...
            } else {alert('Помилка: Викладач '+teacher_name2+' вже зайнятий');
                tr[0].getElementsByClassName("id_group:"+group)[0].getElementsByClassName("subjects")[0].selectedIndex=0;
                tr[1].getElementsByClassName("teacher:"+group)[0].innerHTML='';
                tr[1].getElementsByClassName("audience:"+group)[0].readOnly=true;
...
function addCountSubjectWeek(){
    if(prev_subject_id!=-1) {
        ...
        if ((predmets[prev_arr_index]['dual'] == 1) && (prev_pract == true)) {
            alert(predmets[prev_arr_index]['count_dual']);
            predmets[prev_arr_index]['count_dual'] = predmets[prev_arr_index]['count_dual'] + 0.5;
            alert(predmets[prev_arr_index]['count_dual']);
        }
        else {
            alert(predmets[prev_arr_index]['count_first']);
...
function subCountSubjectFullWeek(index, subject_id, dual_pract){
    if (subject_id!=-1){
        if ((predmets[index]['dual']==1)&&(dual_pract == true)) {
            alert(predmets[index]['count_dual']);
            predmets[index]['count_dual'] = predmets[index]['count_dual'] - 1;
            alert(predmets[index]['count_dual']);
        }
        else {
            alert(predmets[index]['count_first']);
...
function subCountSubjectWeek(index, subject_id, dual_pract){
    if (subject_id!=-1){
        if ((predmets[index]['dual']==1)&&(dual_pract == true)) {
            ...
            alert(predmets[index]['count_first']+' - '+predmets[index]['id']+' - '+predmets[index]['subject_title']+' - '+predmets[index]['group']);
...
function changeAudience(){
    if (!((prev_input_sel === input_sel)&&(input_value===input_sel.value))) {
        ...
        var arr_teacher=[]; var arr_teacher2=[];
        ...
        var arr2_teacher=[]; var arr2_teacher2=[];
...
function peintInputs(rows){
    ...
    for (var i=0; i<rows[1].getElementsByTagName("input").length; i++){
        if(rows[1].getElementsByTagName("input")[i].readOnly==false) {arr_audience1.push(rows[1].getElementsByTagName("input")[i]); arr_sub.push(rows[0].getElementsByTagName("select")[i]);}
    ...
    for (i=0; i<arr_audience1.length; i++){
        for(var j=0; j<arr_audience1.length; j++)
            if ((arr_audience1[i]!=arr_audience1[j])&&(arr_audience1[i].value==arr_audience1[j].value)&&(arr_sub[i].options[arr_sub[i].selectedIndex].value!=arr_sub[j].options[arr_sub[j].selectedIndex].value)) { pochozi.push(arr_audience1[i]); pochozi.push(arr_audience1[j]); }
            else {arr_normal.push(arr_audience1[i]); arr_normal.push(arr_audience1[j]);}

        for (j=0; j<arr_audience2.length; j++){
            if (arr_audience1[i].value==arr_audience2[j].value) { pochozi.push(arr_audience1[i]); pochozi.push(arr_audience2[j]); }
            else {arr_normal.push(arr_audience1[i]); arr_normal.push(arr_audience2[j]);}
        }
    }

    for (i=0; i<arr_audience2.length; i++){
        for(var j=0; j<arr_audience1.length; j++)
            if (arr_audience2[i].value==arr_audience1[j].value) { pochozi.push(arr_audience2[i]); pochozi.push(arr_audience1[j]); }
            else {arr_normal.push(arr_audience2[i]); arr_normal.push(arr_audience1[j]); }

        for (j=0; j<arr_audience2.length; j++){
            if  ((arr_audience2[i]!=arr_audience2[j])&&(arr_audience2[i].value==arr_audience2[j].value)) { pochozi.push(arr_audience2[i]); pochozi.push(arr_audience2[j]); }
            else {arr_normal.push(arr_audience2[i]); arr_normal.push(arr_audience2[j]);}
        }
    }
